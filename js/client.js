...
app.controller("TicTacTicTacToeController", ["$scope", "$firebase", "$firebaseAuth", "$timeout",
  function($scope, $firebase, $firebaseAuth, $timeout) {
    ...
    $scope.auth = $firebaseAuth($scope.rootRef);
    ...
    //         $scope.auth.$getCurrentUser().then(function(loggedInUser) {
    //             if (loggedInUser) {
    ...
    $scope.joinGame = function(provider) {
      $scope.auth.$authWithOAuthRedirect(provider).then(function(authData) {
